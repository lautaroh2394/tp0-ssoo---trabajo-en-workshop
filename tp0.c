...
void configure_logger() {
  ...
  */
  logger = log_create("tp0logs.log", "tp0", true, LOG_LEVEL_TRACE);
}
...
int connect_to_server(char * ip, char * port) {
  ...
  memset(&hints, 0, sizeof(hints));////en primer parametro espera puntnero a void, pero le mando la direccion de hints. que pasa acá
									////borra la direccion y la setea en 0000.. ? SI, CONNECT(..) NECESITA UNO DE SUS PARAMETROS SETEADO TODO EN CERO (Guia beej pag 11)
									////se asegura que el struct esté vacío
  ...
  int server_socket = socket(server_info->ai_family,server_info->ai_socktype,server_info->ai_protocol);////socket(domain, type, protocol) --- gb 5.2

  // 3. Conectemosnos al server a traves del socket! Para eso vamos a usar connect()
  int retorno = connect(server_socket, server_info->ai_addr,server_info->ai_addrlen);////(int sockfd,struct sockaddr *serv_addr, int addrlen) gb 5.4
////sockfd es el servrsocket
////*serv_addr es el campo "ai_addr" de server_info, seteado por getaddrinfo
...
void  wait_hello(int socket) {
  ...
  */
  size_t len = strlen(hola)+1;
  char * buffer = malloc(len);////creo que habría que mandarle el length del hola
  /*
  ...
  */
  int result_recv = recv( socket, buffer, len, NULL);
  ////	       ssize_t recv(int sockfd, void *buf, size_t len, int flags);
////	gb 5.7

  if (result_recv == -1){
	  log_error(logger,"result_recv ERROR");
  }
  else {
	  if (string_equals_ignore_case(buffer, hola)){
		  log_debug(logger,"result_recv EXITO");
	  }
	  else {
		  log_warning(logger,"result_recv recibido incorrecto");
	  }
  }
  /*
...
Alumno read_hello() {
  ...
  */
  ...
  char * ap= readline("Apellido: ");
   memcpy(alumno.apellido,ap, strlen(ap));
...
//  ContentHeader * header = { /* 8.1. */ };
  ...
  int header_recv = recv( socket, header, sizeof(ContentHeader), NULL);
///TODO: VERIFICAR QUE ID == 18
  ...
  */
  ...
  char * espacio = malloc(header->len);
  int body_datos_recv = recv( socket, espacio, header->len, NULL);
